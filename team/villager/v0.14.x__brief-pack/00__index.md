...
| 20 | `20__merge-state-brief.md` | **🔴 URGENT.** Conductor + @Content stuck in conflict loop with no escape. Promote `merge_state.json` to first-class state; new `sgit merge-abort` and `sgit resolve` commands; both Flow A (capture-then-resolve, agent-friendly) and Flow B (resolve-then-merge-commit, git-style) supported; push refuses with `.conflict` files in tree (`--push-conflict` override). | ~1 day | Lands first in remaining queue (Brief 18 already DONE). |
| 21 | `21__brief-20-followups.md` | Brief 20 review follow-ups: `sgit history reset --fetch` flag + 5 missing integration tests from Brief 20 §7b. The conductor agent's recovery report from 2026-05-08 confirmed the gap: standard clones don't cache the parent commit they want to reset to, so the documented "reset to parent" recovery path is unavailable. Without `--fetch` users must full-re-clone. | ~½ day | Reviewer-fix-style pass after Brief 20 merges. |
