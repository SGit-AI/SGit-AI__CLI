...
**Question:** does the loss happen only when the indexed/cached *file* is edited, or also when its *folder(s)* are touched?

**Answer: neither — under Option B the blast radius is the ENTIRE VAULT.** Verified: `Vault__Sync__Commit` calls `_scan_local_directory()` (the **whole** directory) and passes the full `new_file_map` to `sub_tree.build()`, so **every commit regenerates every tree entry in the vault**. Unchanged files normally survive this invisibly because their entries are byte-identical (deterministic metadata encryption + blob reuse) and CAS dedup keeps their tree ids stable — but a field the writing client does not know about is dropped from *every* entry, everywhere.
...
|---|---|---|
| `api-keys/api-key-2` | same id | **CHANGED** |
| `api-keys` | same id | **CHANGED** |
| `docs` | CHANGED (expected) | CHANGED |
| `root` | CHANGED (expected) | CHANGED |

`api-keys/*` is nowhere near the edit, yet its trees change under the old client — because its entries were regenerated without `index_mode`. **One edit to an unrelated file in an unrelated folder erases every declaration in the vault.**
