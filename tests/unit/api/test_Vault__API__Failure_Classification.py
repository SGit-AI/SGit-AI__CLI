...
# ---------------------------------------------------------------------------
# Vault__Sync__Pull._classify_chunk_exception — sibling of _classify_exception
# (Pull side; invoked when batch_read raises before per-file status arrives).
# Must classify 403/404/5xx the same way as the API-side classifier so a
# whole-chunk forbidden response isn't mis-labelled as "server under load".
# ---------------------------------------------------------------------------

class Test_Vault__Sync__Pull__Classify_Chunk_Exception:

    def setup_method(self):
        from sgit_ai.core.actions.pull.Vault__Sync__Pull import Vault__Sync__Pull
        self.pull = Vault__Sync__Pull()

    def test_http_403_classifies_as_forbidden(self):
        err = HTTPError('http://x', 403, 'Forbidden', {}, BytesIO(b''))
        f   = self.pull._classify_chunk_exception('bare/data/obj-cas-imm-aabbccdd1122', err)
        assert f.classification == Enum__Fetch_Failure_Class.FORBIDDEN
