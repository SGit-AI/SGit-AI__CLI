...
class Pull__Workspace(Workflow__Workspace):
    """Adds non-serialisable manager objects needed across all pull steps."""

    sync_client    : object = None   # Vault__Sync__Pull instance
    ...
    def ensure_managers(self, sg_dir: str) -> None:
        ...
        self.merge_helper   = Vault__Merge(crypto=crypto)
        self.fetcher        = Vault__Fetch(crypto=crypto, api=self.sync_client.api,
                                           storage=self.storage)
