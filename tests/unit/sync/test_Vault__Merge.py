...
class Test_Vault__Merge:
    ...
    def _make_flat(self, entries_dict: dict) -> dict:
        """Build a flat dict {path: {'blob_id': str, 'size': int, 'content_hash': str}}."""
        flat = {}
        for path, blob_id in entries_dict.items():
            blob_id = f'obj-cas-imm-{blob_id}' if not blob_id.startswith('obj-cas-imm-') else blob_id
            flat[path] = {'blob_id': blob_id, 'size': 10, 'content_hash': ''}
        return flat

    def test_identical_trees_no_changes(self):
        base   = self._make_flat({'a.txt': 'aabbccddeeff'})
        ours   = self._make_flat({'a.txt': 'aabbccddeeff'})
        theirs = self._make_flat({'a.txt': 'aabbccddeeff'})
        result = self.merger.three_way_merge(base, ours, theirs)
        assert result['conflicts'] == []
        assert result['added']     == []
        assert result['modified']  == []
        assert result['deleted']   == []
        assert len(result['merged_map']) == 1

    def test_theirs_adds_file(self):
        base   = self._make_flat({'a.txt': 'aabbccddeeff'})
        ours   = self._make_flat({'a.txt': 'aabbccddeeff'})
        theirs = self._make_flat({'a.txt': 'aabbccddeeff', 'b.txt': '112233445566'})
        result = self.merger.three_way_merge(base, ours, theirs)
    ...
    def test_theirs_modifies_file(self):
        base   = self._make_flat({'a.txt': 'aabbccddeeff'})
        ours   = self._make_flat({'a.txt': 'aabbccddeeff'})
        theirs = self._make_flat({'a.txt': '112233445566'})
        result = self.merger.three_way_merge(base, ours, theirs)
        assert 'a.txt' in result['modified']
        assert result['conflicts'] == []
        assert result['merged_map']['a.txt']['blob_id'] == 'obj-cas-imm-112233445566'

    def test_ours_modifies_theirs_unchanged(self):
        base   = self._make_flat({'a.txt': 'aabbccddeeff'})
        ours   = self._make_flat({'a.txt': '112233445566'})
        theirs = self._make_flat({'a.txt': 'aabbccddeeff'})
        result = self.merger.three_way_merge(base, ours, theirs)
        assert result['conflicts'] == []
        assert result['merged_map']['a.txt']['blob_id'] == 'obj-cas-imm-112233445566'
    ...
    def test_both_modify_different_content_conflict(self):
        base   = self._make_flat({'a.txt': 'aabbccddeeff'})
        ours   = self._make_flat({'a.txt': '112233445566'})
        theirs = self._make_flat({'a.txt': 'ffeeddccbbaa'})
        result = self.merger.three_way_merge(base, ours, theirs)
        assert 'a.txt' in result['conflicts']

    def test_theirs_deletes_unchanged_file(self):
        base   = self._make_flat({'a.txt': 'aabbccddeeff', 'b.txt': '112233445566'})
        ours   = self._make_flat({'a.txt': 'aabbccddeeff', 'b.txt': '112233445566'})
        theirs = self._make_flat({'a.txt': 'aabbccddeeff'})
        result = self.merger.three_way_merge(base, ours, theirs)
        assert 'b.txt' in result['deleted']
        assert result['conflicts'] == []
        assert 'b.txt' not in result['merged_map']

    def test_theirs_deletes_modified_file_conflict(self):
        base   = self._make_flat({'a.txt': 'aabbccddeeff'})
        ours   = self._make_flat({'a.txt': '112233445566'})
        theirs = self._make_flat({})
        result = self.merger.three_way_merge(base, ours, theirs)
        assert 'a.txt' in result['conflicts']

    def test_both_add_same_file_different_content_conflict(self):
        base   = self._make_flat({})
        ours   = self._make_flat({'new.txt': 'aabbccddeeff'})
        theirs = self._make_flat({'new.txt': '112233445566'})
        result = self.merger.three_way_merge(base, ours, theirs)
    ...
    def test_both_add_same_file_same_content_no_conflict(self):
        base   = self._make_flat({})
        ours   = self._make_flat({'new.txt': 'aabbccddeeff'})
        theirs = self._make_flat({'new.txt': 'aabbccddeeff'})
        result = self.merger.three_way_merge(base, ours, theirs)
        assert result['conflicts'] == []

    def test_both_delete_same_file(self):
        base   = self._make_flat({'a.txt': 'aabbccddeeff'})
        ours   = self._make_flat({})
        theirs = self._make_flat({})
        result = self.merger.three_way_merge(base, ours, theirs)
    ...
    def test_complex_merge(self):
        base   = self._make_flat({'a.txt': 'aabbccddeeff', 'b.txt': '112233445566', 'c.txt': 'ffeeddccbbaa'})
        ours   = self._make_flat({'a.txt': 'aabbccddeeff', 'b.txt': 'aaaa11112222', 'd.txt': '999888777666'})
        theirs = self._make_flat({'a.txt': '000000000000', 'c.txt': 'ffeeddccbbaa', 'e.txt': '555444333222'})
        result = self.merger.three_way_merge(base, ours, theirs)
        ...
        merged = result['merged_map']
        assert 'a.txt' in merged                    # theirs version
        assert 'b.txt' in merged                    # ours modified (kept in conflict)
        assert 'c.txt' not in merged                # theirs deleted, ours unchanged
        assert 'd.txt' in merged                    # ours added
        assert 'e.txt' in merged                    # theirs added
    ...
    def test_empty_trees(self):
        ...
        result = self.merger.three_way_merge(base, ours, theirs)
        assert result['conflicts'] == []
        assert len(result['merged_map']) == 0
    ...
    def test_ours_deletes_theirs_modifies_conflict(self):
        ...
        result = self.merger.three_way_merge(base, ours, theirs)
        assert 'a.txt' in result['conflicts']
        assert 'a.txt' in result['merged_map']
    ...
    def test_ours_deletes_theirs_unchanged(self):
        ...
        result = self.merger.three_way_merge(base, ours, theirs)
        assert result['conflicts'] == []
        assert 'a.txt' not in result['merged_map']
...
class Test_Vault__Merge__Conflict_Files:
    ...
    def test_write_conflict_files_creates_files(self):
        ...
        theirs_flat = self._make_flat({'a.txt': blob_id})
        conflicts  = ['a.txt']
